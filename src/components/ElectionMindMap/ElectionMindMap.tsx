...
export default function ElectionMindMap() {
  ...
  const activeNodeData = MIND_NODES.find((n) => n.id === activeNode);
          ...
          <svg className={styles.connectionsSvg} viewBox="0 0 100 100" preserveAspectRatio="none">
            {activeNodeData &&
              relatedNodes.map((relatedId) => {
                const relatedNode = MIND_NODES.find((n) => n.id === relatedId);
            ...
            {!activeNode &&
              filteredNodes.map((node) =>
                node.related
                  .filter((relatedId) => {
                    const related = MIND_NODES.find((n) => n.id === relatedId);
