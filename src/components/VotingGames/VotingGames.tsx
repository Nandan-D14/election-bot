...
const TRUE_FALSE_QUESTIONS = [
  { id: 'q1', question: 'You can vote without a Voter ID if you have Aadhaar?', answer: true, explanation: 'Aadhaar is an approved alternative ID for voting.' },
  { id: 'q2', question: 'You can vote multiple times if you register in different constituencies?', answer: false, explanation: 'Duplicate voting is illegal and punishable.' },
  { id: 'q3', question: 'Mobile phones are allowed inside the voting booth?', answer: false, explanation: 'Phones must be switched off or left outside.' },
  { id: 'q4', question: 'NOTA means you reject all candidates?', answer: true, explanation: 'None Of The Above is your right to reject all options.' },
  { id: 'q5', question: 'Campaigning is allowed 50 meters from polling stations?', answer: false, explanation: 'No campaigning within 100 meters is allowed.' },
  { id: 'q6', question: 'You must be 18+ years old on the qualifying date to vote?', answer: true, explanation: 'Age is calculated on the date set by Election Commission.' },
];
...
const ID_ITEMS = [
  ...
  { id: 'id4', name: 'Student ID', icon: '🎓', isValid: false, reason: 'Not a government-issued photo ID' },
  { id: 'id5', name: 'Passport', icon: '🛂', isValid: true },
  { id: 'id6', name: 'Driving License', icon: '🚗', isValid: true },
  { id: 'id7', name: 'Library Card', icon: '📚', isValid: false, reason: 'Not an official government ID' },
...
export default function VotingGames() {
  ...
  const getScore = (game: string) => scores.find((s) => s.game === game);
        ...
        <div className={styles.header}>
          ...
          {scores.length > 0 && (
            <div className={styles.totalScore}>
              <span className={styles.scoreBadge}>🏆 Total: {totalScore}/{maxPossible}</span>
            </div>
          )}
        </div>
...
// --- STEP ORDER GAME ---
function StepOrderGame({ onComplete }: { onComplete: (game: string, score: number, total: number) => void }) {
      ...
      {showResult ? (
        <div className={styles.resultBox}>
          ...
          <p className={styles.resultMessage}>
            {score === items.length ? '🎉 Perfect! You know the voting process!' : 'Keep practicing to learn the correct order!'}
          </p>
          <button className={styles.playAgainButton} onClick={reset}>Play Again</button>
        </div>
      ) : (
        <button className={styles.checkButton} onClick={checkAnswer}>Check Answer</button>
      )}
    </div>
  );
}
...
// --- TRUE/FALSE GAME ---
function TrueFalseGame({ onComplete }: { onComplete: (game: string, score: number, total: number) => void }) {
        ...
        <div className={styles.resultBox}>
          ...
          <div className={styles.tfSummary}>
            {TRUE_FALSE_QUESTIONS.map((q, i) => (
              <div key={q.id} className={`${styles.tfItem} ${answers[i] === q.answer ? styles.tfCorrect : styles.tfWrong}`}>
  ...
  return (
    <div className={styles.gameContainer}>
      ...
      <div className={styles.progressBar}>
        <div className={styles.progressFill} style={{ width: `${(currentQ / TRUE_FALSE_QUESTIONS.length) * 100}%` }} />
      </div>
      <div className={styles.tfQuestionBox}>
        <p className={styles.tfQuestion}>{q.question}</p>
        <div className={styles.tfButtons}>
          <button className={`${styles.tfButton} ${styles.tfTrue}`} onClick={() => answer(true)}>TRUE</button>
          <button className={`${styles.tfButton} ${styles.tfFalse}`} onClick={() => answer(false)}>FALSE</button>
        </div>
      </div>
      <p className={styles.questionCounter}>Question {currentQ + 1} of {TRUE_FALSE_QUESTIONS.length}</p>
    </div>
  );
}

// --- ID QUIZ GAME ---
function IDQuizGame({ onComplete }: { onComplete: (game: string, score: number, total: number) => void }) {
  ...
  const score = showResult ? ID_ITEMS.filter((item) => checked.has(item.id) === item.isValid).length : 0;
  ...
  return (
    <div className={styles.gameContainer}>
        ...
        {ID_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              {showResult && item.reason && (
                <span className={styles.idReason}>{item.reason}</span>
              )}
            </button>
          );
        })}
      ...
      {showResult ? (
        <div className={styles.resultBox}>
          <div className={styles.resultScore}>Score: {score}/{ID_ITEMS.length}</div>
          <button className={styles.playAgainButton} onClick={reset}>Play Again</button>
        </div>
      ) : (
...
// --- EVM CHALLENGE GAME ---
function EVMChallengeGame({ onComplete }: { onComplete: (game: string, score: number, total: number) => void }) {
    ...
    return (
      <div className={styles.gameContainer}>
        ...
        <div className={styles.resultBox}>
          ...
          <button className={styles.playAgainButton} onClick={reset}>Practice Again</button>
        </div>
      </div>
    );
  }
          ...
          <div className={styles.vvpatScreen}>
            {verified ? (
              <div className={styles.vvpatVerified}>
                ...
                <p>Vote Verified!</p>
                <small>You voted for: {CANDIDATES.find(c => c.id === selectedCandidate)?.symbol}</small>
              </div>
            ) : selectedCandidate ? (
