...
// Hoisted cable curves — avoid re-creating geometry objects on every render
...
// VVPAT Component
// --------------------------------------------------------
// Pre-built map to avoid O(N) array scans during the 3D render loop
const CANDIDATE_MAP = new Map(SIMULATED_CANDIDATES.map((c) => [c.id, c]));
...
function VvpatUnit({
  ...
  const candidate = selectedCandidate ? CANDIDATE_MAP.get(selectedCandidate) : undefined;
        ...
        >
          ...
          <Float speed={1.5} rotationIntensity={0.1} floatIntensity={0.2}>
            <group position={[0, -0.5, 0]}>
              ...
              {/* Cables — using hoisted curve constants */}
              <mesh position={[-1.8, 0.2, -1]}>
                <tubeGeometry args={[cable1Curve, 20, 0.05, 8, false]} />
                <meshStandardMaterial color="#222" />
              </mesh>
              <mesh position={[1.8, 0.2, -1]}>
                <tubeGeometry args={[cable2Curve, 20, 0.05, 8, false]} />
                <meshStandardMaterial color="#222" />
              </mesh>
            </group>
          </Float>
