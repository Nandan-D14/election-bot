...
export function useActivityScores() {
  ...
  // Guard: only write to localStorage after initial load is complete
  ...
  // Memoize derived values — only recalculate when scores actually change
  const totalPoints = useMemo(
    () => ACTIVITIES.reduce((sum, a) => sum + (scores[a.key] ? a.points : 0), 0),
    [scores]
  );
