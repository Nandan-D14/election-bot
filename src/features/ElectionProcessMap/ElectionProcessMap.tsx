...
const PHASE_MAP = new Map(ELECTION_PHASES.map((p) => [p.id, p]));
...
export default function ElectionProcessMap() {
  ...
  const activePhaseData = activePhase ? PHASE_MAP.get(activePhase) : undefined;
            ...
            <div className={styles.flowchartView}>
              <svg ref={svgRef} className={styles.flowchartSvg} viewBox="0 0 1000 600">
                ...
                {CONNECTING_LINES.map((line, idx) => {
                  const fromPhase = PHASE_MAP.get(line.from);
                  const toPhase = PHASE_MAP.get(line.to);
